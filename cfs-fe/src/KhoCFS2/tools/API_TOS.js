...
class APITOS extends React.Component {
  ...
  handleViewData() {
    let dataSend = {
      ...
      MES_STATUS: this.state.data.MES_STATUS,
      FUNCTION_PATCH: this.state.data.FUNCTION_PATCH,
    }

    fetch(window.root_url + `api-tos/view`, {
      method: "POST",
      headers: {
      ...
      .then((response) => {
        if (response.Status) {
          let tempPayload = response.Payload.map(item => {
            item.POST_DATA = JSON.parse(item.POST_DATA);
            item.GET_DATA = JSON.parse(item.GET_DATA);
            return item;
          })
          let temp = this.createRows(tempPayload);
          this.setState({
            dataTable: temp,
            alert: {
              isOpen: true,
