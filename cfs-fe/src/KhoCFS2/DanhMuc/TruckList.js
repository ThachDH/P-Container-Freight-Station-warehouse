...
class TruckList extends React.Component {
  ...
  handleViewData() {
    let url = window.root_url + `bs-trucks/view`;
    fetch(url, {
      method: "POST",
      headers: {
  ...
  handleDelete() {
    let url = window.root_url + `bs-trucks/delete`;
    let { dataTable } = this.state;
  ...
  handleSave() {
    let url = window.root_url + `bs-trucks/InsertAndUpdate`;
    let { dataTable } = this.state;
    let dataSend = dataTable.filter(p => p.status === 'insert' || p.status === 'update').map(data => {
      // data['TRUCK_DATE_EXP'] = moment(data.TRUCK_DATE_EXP, "YYYY/MM/DD[T]HH:mm:ss").add(1, 'days');
      ...
      .then(response => {
        if (response.Status) {
          let newDatas = dataTable.map((item, index) => {
            let returnValue = response.Payload.find(p => p.TRUCK_NO === item.TRUCK_NO);
