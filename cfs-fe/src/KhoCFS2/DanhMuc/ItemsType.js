...
class ItemsType extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  handleDelete() {
    let url = window.root_url + `bs-equipments-types/delete`;
    let { dataTable } = this.state;
  ...
  handleSave() {
    let url = window.root_url + `bs-equipments-types/InsertAndUpdate`;
    let { dataTable } = this.state;
    let checkColumn = {
      ...
      .then(response => {
        if (response.Status) {
          let newDatas = dataTable.map((item, index) => {
            let returnValue = response.Payload.find(p => p.EQU_TYPE === item.EQU_TYPE && p.EQU_TYPE === item.EQU_TYPE);
...
ItemsType.defaultProps = { multiple: true };
