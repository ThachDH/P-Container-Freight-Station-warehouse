...
class MinValueConfig extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            confirmPopup: {
    ...
    componentDidMount() {
        let url = window.root_url + `config-min-value/view`;
        fetch(url, {
            method: "POST",
            headers: {
    ...
    handleDelete() {
        let url = window.root_url + `config-min-value/delete`;
        let { dataTable } = this.state;
    ...
    handleSave() {
        let url = window.root_url + `config-min-value/insertAndUpdate`;
        let { dataTable } = this.state;
        ...
        let dataSend = dataTable.filter(p => p.status === 'insert' || p.status === 'update').map(data => {
            ...
            if (data.CNTRSZTP.length === 0 || data.UNIT_INVOICE.length === 0) {
        ...
        fetch(url, {
            ...
            .then(response => {
                if (response.Status) {
                    let newDatas = dataTable.map((item, idx) => {
                        let returnValue = response.Payload.find(p => p.CNTRSZTP === item.CNTRSZTP && p.UNIT_INVOICE === item.UNIT_INVOICE);
                                ...
                                <Grid item mt={1} md={12}>
                                    <DataGrid
                                        ...
                                        rows={(this.state.dataTable)
                                            .filter(data => data.CNTRSZTP.toUpperCase().includes(this.state.searchField.CNTRSZTP.toUpperCase()))
                                        }
                                        rowHeight={35}
