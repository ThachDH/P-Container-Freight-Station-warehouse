...
class DiscountConfig extends React.Component {
  ...
  handleDelete() {
    ...
    let url = window.root_url + `config-discounts/delete`;
  ...
  handleSelectForm(formData) {
    let url = window.root_url + `config-discounts/viewConfDisTemp`;
    let dataSend = {
      NAME: formData.NAME
    }
    fetch(url, {
  ...
  handleSave() {
    let { dataTable } = this.state;
    let url = window.root_url + `config-discounts/insertAndUpdate`;
