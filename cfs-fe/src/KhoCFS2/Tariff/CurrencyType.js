...
class Currency extends React.Component {
  constructor(props) {
    ...
    this.columns = [
      ...
      {
        field: "CURRENCY_CODE",
        headerName: 'Mã loại tiền',
        flex: 1,
        editable: true
      },
      {
        field: "CURRENCY_NAME",
        headerName: "Tên loại tiền",
        flex: 1,
        editable: true
      },
      {
  ...
  handleViewData() {
    let url = window.root_url + `currency-type/view`;
    fetch(url, {
      method: "POST",
      headers: {
  ...
  handleSave() {
    let url = window.root_url + `currency-type/InsertAndUpdate`;
    let { dataTable } = this.state;
    ...
    let dataSend = dataTable.filter(p => p.status === 'insert' || p.status === 'update').map(data => {
      ...
      if (data.CURRENCY_CODE.length === 0) {
    ...
    if (!checkEmptySend) {
      this.setState({
        alert: {
          isOpen: true,
          duration: 3000,
          message: "Mã loại tiền không được phép để trống",
          type: "error"
        }
      });
      return;
      ...
      .then(response => {
        if (response.Status) {
          let newDatas = dataTable.map((item, index) => {
            let returnValue = response.Payload.find(p => p.CURRENCY_CODE === item.CURRENCY_CODE && p.CURRENCY_NAME === item.CURRENCY_NAME);
  ...
  handleDelete() {
    let url = window.root_url + `currency-type/delete`;
    let { dataTable } = this.state;
                ...
                <Grid item mt={1} md={12}>
                  <DataGrid
                    ...
                    rows={(this.state.dataTable)
                      .filter(data => data.CURRENCY_CODE.toUpperCase().includes(this.state.tableFilter.CURRENCY_CODE.toUpperCase()))
                    }
                    rowHeight={35}
...
Currency.defaultProps = { multiple: true };
