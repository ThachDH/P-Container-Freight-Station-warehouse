...
class ChiTietChucVu extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      data: initalData,
  ...
  checkTrangthai(type) {
    switch (type) {
      case 0: this.insertJobTitle(); break;
      case 1: this.updateJobTitle(); break;
      default: return;
    }
  }

  //Thoilc(*Note)-Insert dữ liệu chức vụ
  insertJobTitle() {
    let url = window.root_url + `job-titles/insert`;
    let dataSend = {
  ...
  //Thoilc(*Note)-Update dữ liệu chức vụ
  updateJobTitle() {
    let url = window.root_url + `job-titles/update`;
    let dataSend = {
      No_: this.state.data.No_,
      Name: this.state.data.Name,
    };
