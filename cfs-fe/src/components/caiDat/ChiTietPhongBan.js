...
class ChiTietPhongBan extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      data: initalData,
  ...
  checkTrangthai(type) {
    switch (type) {
      case 0: this.insertDepartment(); break;
      case 1: this.updateDepartment(); break;
      default: return;
    }
  }

  //Thoilc(*Note)-Insert dữ liệu phòng ban
  insertDepartment() {
    let url = window.root_url + `departments/insert`;
    let dataSend = {
  ...
  //Thoilc(*Note)-Update dữ liệu phòng ban
  updateDepartment() {
    let url = window.root_url + `departments/update`;
    let dataSend = {
      No_: this.state.data.No_,
      Name: this.state.data.Name,
    };
