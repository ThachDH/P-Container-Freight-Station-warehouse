...
class ChiTietDVT extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      data: initalData,
  ...
  checkTrangthai(type) {
    switch (type) {
      case 0: this.insertUnit(); break;
      case 1: this.updateUnit(); break;
      default: return;
    }
  }

  //Thoilc(*Note)-Insert dữ liệu đơn vị tính
  insertUnit() {
    let url = window.root_url + `unit-of-measures/insert`;
    let dataSend = {
  ...
  //Thoilc(*Note)-Update dữ liệu đơn vị tính
  updateUnit() {
    let url = window.root_url + `unit-of-measures/update`;
    let dataSend = {
      No_: this.state.data.No_,
      Name: this.state.data.Name,
    };
