...
class ChonPallet extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      page: 0,
  ...
  //Binh - lưu và render những pallet đã chọn ra ds pallet
  selected() {
    if (this.props.dialog.type === 1) {
      let SelectedPallet = this.state.dataTable.filter(
        item => item.isChecked === true,
      );
      SelectedPallet.map(item1 => {
        return item1.isChecked === true ? item1.isSaved : false;
      })
      this.setState({
  ...
  //Binh(*note) // khi chọn phiếu khác - sẽ setState lại tất cả các checkbox = false //
  componentWillReceiveProps(nextProps) {
    if (this.props.dialog.type === 1) {
      this.props.handleSavePallet.map(item => {
        return nextProps.handleSavePallet.map(item1 => {
          if (item1.isSaved !== item.isSaved) {
            let newTableData = [...this.state.dataTable];
            newTableData.map((item3) => (item3.isSaved = true));
  ...
  componentDidUpdate(prevProps, prevState) {
    if (this.props.dialog.type === 1) {
      if (prevProps.phieuXuatKho.journalExport_No_ !== this.props.phieuXuatKho.journalExport_No_) {
        this.setState({
          dataTable: [],
          fromDate: moment().subtract(1, "days"),
          toDate: moment(),
              ...
              <Stack direction="row" spacing={1}>
                ...
                <Stack direction="row" spacing={0.5}>
                  ...
                  <Button
                    type="button"
                    variant="contained"
                    // endIcon={<SendIcon sx={{ fontSize: 10 }} />}
                    // sx={{ marginTop: 1, marginLeft: 40, fontSize: 5 }}
                          ...
                          return (
                            ...
                            >
                              <TableCell align="center">
                                <Checkbox
                                  disabled={row.isSaved === true ? true : false}
                                  checked={
                                    row.isChecked === true
                                  }
                                  name="chon-pallet"
...
ChonPallet.defaultProps = { multiple: true };
export default ChonPallet;
